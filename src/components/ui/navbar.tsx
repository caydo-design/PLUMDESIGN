...
export function Navbar({
  ...
  return (
    <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-foreground/5">
      <div className="px-6 md:px-12 lg:px-20 py-4 lg:py-5 max-w-[1440px] mx-auto w-full">
        <div className="flex items-center justify-between">
          <a href="/">
            <Logo />
          </a>
          <div className="hidden lg:flex items-center gap-10 text-[15px] font-medium text-foreground/70">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
      ...
      {open && (
        <div className="lg:hidden border-t border-foreground/5 px-6 md:px-12 py-6 bg-background/95 backdrop-blur-xl">
...
export function BackNavbar({ backHref = "/" }: { backHref?: string }) {
  return (
    <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-foreground/5">
      <div className="flex items-center justify-between px-6 md:px-12 lg:px-20 py-4 lg:py-5 max-w-[1440px] mx-auto w-full">
        <a href="/">
          <Logo />
        </a>
        <a
